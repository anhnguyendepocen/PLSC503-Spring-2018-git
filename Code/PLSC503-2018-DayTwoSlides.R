##############################################
# PLSC 503 Day two slides (2018 remix).
...
temp<-getURL("https://raw.githubusercontent.com/PrisonRodeo/PLSC503-Spring-2018-git/master/Data/CountryData2000.csv")
IR2000<-read.csv(text=temp, header=TRUE)
rm(temp)

# attach(IR2000)  # <-- as a rule, you shouldn't do this

# I offer no warranty that any of this is the most 
# efficient way to do what I'm doing... all graphics
# are using base R graphics funcitonality, with
# the exception of the scatterplot matrices.
...
par(cex=1.2)
par(mar=c(4,4,2,2))
with(IR2000, plot(lifeexpectancy,infantmortalityperK,pch="",xlab="Life Expectancy at Birth",
